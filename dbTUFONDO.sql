...
CREATE TABLE IF NOT EXISTS workshopVargas.maestra_pqrs (
	tipo_inci VARCHAR(3) NOT NULL,
    motivo_inci VARCHAR(3) NOT NULL,
    descripcion VARCHAR(100),     
    cod_usuario VARCHAR(6),    
  PRIMARY KEY (tipo_inci),
  CONSTRAINT fk_usu FOREIGN KEY (cod_usuario) REFERENCES usuarios (cod_usuario) ON DELETE RESTRICT ON UPDATE CASCADE
);
CREATE TABLE IF NOT EXISTS workshopVargas.tipificacion_pqrs (
	id INT AUTO_INCREMENT,
	tipo_inci VARCHAR(3) NOT NULL,
    descripcion_tipo VARCHAR(30),
    motivo_inci VARCHAR(3) NOT NULL,     
    descripcion_motivo VARCHAR(100),         
    cod_usuario VARCHAR(6),
  PRIMARY KEY (id),  
  CONSTRAINT fk_tipo_inci FOREIGN KEY (tipo_inci) REFERENCES maestra_pqrs (tipo_inci) ON DELETE RESTRICT ON UPDATE CASCADE  
);
...
(13,'40020243','C','Valiente','Acuna','Lina','Marcela','','F','93-12-25','170','10000','Admin'),
...
(11,'40020243','Valiente Acuna Lina Marcela','','20-08-23','100','200','Q',5500000,'A'),
...
INSERT INTO maestra_pqrs VALUES 
('100','200','Solicitud de Estado de Cuenta','Admin'),
('101','200','Queja','Admin'),
('102','203','Sugerencia','Admin'),
('103','203','Felicitaciones al Servicio','Admin');

INSERT INTO tipificacion_pqrs VALUES 
(1,'100','Solicitud de Informacion','200','Certificados','Admin'),
(2,'101','Queja','201','Inconformidad','Admin'),
(3,'102','Sugerencias','202','Varias','Admin'),
(4,'103','OTRA','203','Otras razones','Admin');
...
-- 6.Vista de los ultimos 5 asociados que ingresaron como afiliados al fondo de empleados
...
-- 7. Vista para realizar reporte de ingresos o afiliaciones de asociados por empresa en cada periodo de tiempo
CREATE OR REPLACE VIEW patronales_view AS				
(	
SELECT 			
		a.numdoc		
		,a.nombres		
        ...
        ,a.f_afiliacion
FROM asociados AS a			
JOIN empresas AS e ON a.id_nomina = e.id_nomina
);
...
DELIMITER $$		
CREATE PROCEDURE insertar_ciudad(IN p_cod_ciudad VARCHAR(5), 
			                     IN p_nombre_ciudad VARCHAR(50),
                                 IN p_cod_pais VARCHAR(3) 
            )
BEGIN
...
DELIMITER $$		
CREATE PROCEDURE ordenamiento(IN p_campo VARCHAR(20), 
...
-- 3.SP Esta creado para insertar el movimiento contable de los desembolsos de créditos

DELIMITER $$		
CREATE PROCEDURE desembolso_credito(IN p_cod_credito INT, 
			                     IN p_valor_credito DECIMAL(10,0)
                                 )
BEGIN
	 IF ((SELECT cod_credito FROM det_contable WHERE cod_credito = p_cod_credito AND k_codcon = '27500103' ) IS NOT NULL ) THEN
	 SELECT 'ERROR: El crédito ya tiene un desembolso registrado' AS Error;
     ELSEIF ((SELECT cod_credito FROM credito WHERE cod_credito = p_cod_credito ) IS NULL ) THEN
	 SELECT 'ERROR: El crédito no existe en la tabla de creditos' AS Error;
     ELSE
     INSERT INTO det_contable VALUES 
     (NULL,'OB','11100501',YEAR(CURDATE()),p_cod_credito,NULL,'DESEMBOLSO CREDITO',0,p_valor_credito,CURDATE(),'N'),
     (NULL,'OB','27500103',YEAR(CURDATE()),p_cod_credito,NULL,'DESEMBOLSO CREDITO',p_valor_credito,0,CURDATE(),'N');
     SELECT * FROM det_contable WHERE cod_credito = p_cod_credito;		
    END IF;		
END $$
DELIMITER ;

-- 4.SP Esta creado para la cracion de asociados activos en el fondo de empleados

DELIMITER $$		
CREATE PROCEDURE creacion_asociado(IN p_id INT,IN p_numdoc VARCHAR(50),IN p_nombres VARCHAR(200),IN p_n_razons VARCHAR(200),
			                     IN p_tipo_aso VARCHAR(3),IN p_nomina VARCHAR(200),IN p_periodicidad VARCHAR(1),IN p_salario DECIMAL(10,0)
                                 )
BEGIN
	 IF ((SELECT numdoc FROM cliente WHERE numdoc = p_numdoc) IS NULL ) THEN
	 SELECT 'ERROR: El documento ingresado No se encuentra creado como cliente' AS Error;
     ELSEIF ((SELECT numdoc FROM asociados WHERE numdoc = p_numdoc ) IS NOT NULL ) THEN
	 SELECT 'ERROR: El documento ingresado ya se encuentra registrado como asociado' AS Error;
     ELSE
     INSERT INTO asociados VALUES 
     (p_id,p_numdoc,p_nombres,p_n_razons,CURDATE(),p_tipo_aso,p_nomina,p_periodicidad,p_salario,'A');
     SELECT * FROM asociados WHERE numdoc = p_numdoc;		
    END IF;		
END $$
DELIMITER ;
-- Creación de tablas log

CREATE TABLE IF NOT EXISTS workshopVargas.credito_log (
	cod_credito INT,
    id INT NULL,     
    fecha_credito DATE,
    cod_modalidad VARCHAR(3) NULL,
    description VARCHAR(50) NULL,
    cod_usuario_log VARCHAR(100) NULL,
    fecha_registro DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (cod_credito)
);
CREATE TABLE IF NOT EXISTS workshopVargas.det_contable_log (
	serie INT AUTO_INCREMENT,
    tipo_documento VARCHAR(3) NULL,
    k_codcon VARCHAR(16) NULL,
    k_ano YEAR NULL,
    cod_credito INT,
    id_otrods INT,
    n_movimi VARCHAR(40) NULL,
    cr_peso NUMERIC  NULL,
    db_peso NUMERIC  NULL,
    f_movimi DATE NULL,
    i_anulado VARCHAR(1),
    cod_usuario_log VARCHAR(100) NULL,
    fecha_registro DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS workshopVargas.cliente_log (
	id INT AUTO_INCREMENT,
    numdoc VARCHAR(50) NULL,
    i_tipdoc VARCHAR(1) NULL,
    n_apell1 VARCHAR(50) NULL,
    n_apell2 VARCHAR(50) NULL,
    n_nombr1 VARCHAR(50) NULL,
    n_nombr2 VARCHAR(50) NULL,
    n_razons VARCHAR(50),
    i_sexo VARCHAR(1) NULL,
    f_nacimi DATE,
    cod_pais_nacimi VARCHAR(3) NULL,
    cod_ciudad_nacimi VARCHAR(8) NULL,
    cod_usuario_log VARCHAR(50) NULL,
    fecha_registro DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id)
    );
    CREATE TABLE IF NOT EXISTS workshopVargas.det_contable_log_up (
	serie INT AUTO_INCREMENT,
    tipo_documento VARCHAR(3) NULL,
    k_codcon VARCHAR(16) NULL,
    k_ano YEAR NULL,
    ...
    n_movimi VARCHAR(40) NULL,
    cr_peso NUMERIC  NULL,
    cr_peso_old NUMERIC  NULL,
    db_peso NUMERIC  NULL,
    db_peso_old NUMERIC  NULL,
    f_movimi DATE NULL,
    i_anulado VARCHAR(1),
    cod_usuario_log VARCHAR(50) NULL,
    fecha_registro DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
...
-- 1 Trigger credito BEFORE UPDATE, Se crea trigger para controlar los valores aplicados a la cartera de créditos

DELIMITER $$
USE `workshopvargas`$$
CREATE DEFINER = CURRENT_USER TRIGGER `workshopvargas`.`credito_BEFORE_UPDATE` BEFORE UPDATE ON `credito` FOR EACH ROW
BEGIN
INSERT INTO credito_log
VALUES (OLD.cod_credito,OLD.id,OLD.fecha_credito,OLD.cod_modalidad,'CREDITO ALTERADO',USER(),current_time());
END$$
DELIMITER ;

-- 2 Trigger cliente AFTER INSERT, Se crea trigger para crear respaldo de clientes creados en tabla cliente_log

DELIMITER $$
USE `workshopvargas`$$
CREATE DEFINER = CURRENT_USER TRIGGER `workshopvargas`.`cliente_AFTER_INSERT` AFTER INSERT ON `cliente` FOR EACH ROW
BEGIN
INSERT INTO cliente_log (id,numdoc,i_tipdoc,n_apell1,n_apell2,n_nombr1,n_nombr2,n_razons,i_sexo,f_nacimi,cod_pais_nacimi,cod_ciudad_nacimi,cod_usuario_log,fecha_registro)
VALUES(NEW.id,NEW.numdoc,NEW.i_tipdoc,NEW.n_apell1,NEW.n_apell2,NEW.n_nombr1,NEW.n_nombr2,NEW.n_razons,NEW.i_sexo,NEW.f_nacimi,NEW.cod_pais_nacimi,NEW.cod_ciudad_nacimi,USER(),current_time());
END$$
DELIMITER ;

-- 3 Trigger ahorros AFTER INSERT, Se crea trigger para insertar los registros contables cada vez en la tabla de ahorros existe un movimiento a los aportes

DELIMITER $$
USE `workshopvargas`$$
CREATE DEFINER = CURRENT_USER TRIGGER `workshopvargas`.`ahorros_AFTER_INSERT` AFTER INSERT ON `ahorros` FOR EACH ROW
BEGIN
INSERT INTO det_contable 
VALUES (NULL,'AP',31100505,YEAR(CURDATE()),NULL,NEW.id,'CONTABILIZACION APORTES',NEW.valor_aporte,0,CURDATE(),'N'),
	   (NULL,'AP',11100501,YEAR(CURDATE()),NULL,NEW.id,'CONTABILIZACION APORTES',0,NEW.valor_aporte,CURDATE(),'N');
END$$
DELIMITER ;

-- 4 Trigger det_contable AFTER INSERT, Se crea trigger para tener copia de los registros contables realizados 

DELIMITER $$
USE `workshopvargas`$$
CREATE DEFINER = CURRENT_USER TRIGGER `workshopvargas`.`det_contable_AFTER_INSERT` AFTER INSERT ON `det_contable` FOR EACH ROW
BEGIN
INSERT INTO det_contable_log VALUE (NEW.serie,NEW.tipo_documento,NEW.k_codcon,NEW.k_ano,NEW.cod_credito,NEW.id_otrods,NEW.n_movimi,NEW.cr_peso,NEW.db_peso,NEW.f_movimi,NEW.i_anulado,USER(),CURRENT_TIME());
END$$
DELIMITER ;

-- 5 Trigger det_contable BEFORE UPDATE, Se crea trigger para tener el control de lo registros actualizados en la tabla de movimientos contables

DELIMITER $$
USE `workshopvargas`$$
CREATE DEFINER = CURRENT_USER TRIGGER `workshopvargas`.`det_contable_BEFORE_UPDATE` BEFORE UPDATE ON `det_contable` FOR EACH ROW
BEGIN
INSERT INTO det_contable_log_up (serie,tipo_documento,k_codcon,k_ano,cod_credito,id_otrods,n_movimi,cr_peso,cr_peso_old,db_peso,db_peso_old,f_movimi,i_anulado,cod_usuario_log,fecha_registro)
VALUE (OLD.serie,OLD.tipo_documento,OLD.k_codcon,OLD.k_ano,OLD.cod_credito,OLD.id_otrods,OLD.n_movimi,NEW.cr_peso,OLD.db_peso,NEW.cr_peso,OLD.db_peso,OLD.f_movimi,OLD.i_anulado,USER(),CURRENT_TIME());
END$$
DELIMITER ;